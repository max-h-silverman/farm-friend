...
// farmer cannot reach the prepared SIGNUP text until they have ticked the agreement AND the
// server has recorded it. If the text were reachable first, a farmer could send the SIGNUP
...
const SIGNUP_URL = `sms:+12065551234?body=SIGNUP%20${TOKEN}`;
...
describe("onboarding agreement step", () => {
  it("states message frequency, that rates apply, and how to stop", () => {
    render(<AgreementStep token={TOKEN} signupUrl={SIGNUP_URL} />);
  ...
  it("does not offer the prepared text until the farmer agrees", () => {
    ...
    expect(screen.queryByRole("link", { name: /text signup/i })).not.toBeInTheDocument();
  ...
  it("records the agreement and only THEN reveals the prepared text", async () => {
    ...
    const link = await screen.findByRole("link", { name: /text signup/i });
    expect(link).toHaveAttribute("href", SIGNUP_URL);
  });

  it("keeps the prepared text hidden when the server refuses", async () => {
    // A dead invitation. Revealing the text anyway would send the farmer to a SIGNUP that
  ...
  it("un-ticking hides the prepared text again", async () => {
    ...
    await userEvent.click(screen.getByRole("checkbox"));
    expect(await screen.findByRole("link", { name: /text signup/i })).toBeVisible();
  ...
  it("shows the typed-out instruction when no prepared link can be built", async () => {
    ...
    stubFetch({ ok: true, status: 200 });
    render(<AgreementStep token={TOKEN} signupUrl={null} />);

    await userEvent.click(screen.getByRole("checkbox"));

    expect(await screen.findByText(new RegExp(`SIGNUP ${TOKEN}`))).toBeVisible();
    expect(screen.queryByRole("link", { name: /text signup/i })).not.toBeInTheDocument();
  });
});
