...
Two axes describe an opportunity (see `docs/activity-model-redesign.md`, shipped 2026-05-31):
- **`kind`** (`shift` | `pickup`) — the load-bearing axis that drives outreach pacing, lead times, claim/race semantics, vehicle need, and the post-event flow. Branched on across the codebase; do not rename or repurpose it.
- **`purpose`** (`gleaning` | `farm_help`) — *why* the opportunity exists, orthogonal to `kind`. `gleaning` = food-access / waste reduction (leftovers/surplus, usually for a food bank); `farm_help` = general support for the farm's own work. Defaults to `farm_help`. This is the mute/routing axis: `STOP gleaning` / `STOP farm help` mute by purpose; `STOP {farm}` mutes a farm.
- **`activity_detail`** (free text) — *what* the work is, in the farmer's own words, display-cased ("Inoculate Shiitake Logs"). **There is no fixed activity vocabulary.** Any non-empty `activity_detail` satisfies the required "activity" axis (`parser.py:REQUIRED_SHIFT_FIELDS`). The agent captures what the farmer said and never invents one; "tomatoes" (a crop, not work) still triggers one clarify, but any stated work — however non-standard — is valid and never flagged as "unknown." (`activity_tags` remains on the docs as a deprecated, unread field; remove post-pilot.)

- **Volunteer Shift** (`kind=shift`) — timed, headcount-bounded work at the farm.
- **Surplus Pickup** (`kind=pickup`) — single-claim race for already-set-aside produce that needs to be picked up and taken to a destination (food bank, community fridge, mutual aid). Faster outreach pacing (perishable). Often needs a vehicle.
...
1. Fully open / inspectable / nonprofit-aligned models (OLMo / Ai2). This is the ethical benchmark, and it stays #1 as a *preference*. **But OLMo is not viable at pilot scale — this was tested, not assumed (2026-06-01).** No provider hosts it (DeepInfra serves Gemma in its place; OpenRouter has zero live instruct endpoints). Self-hosting on scale-to-zero serverless GPU was benchmarked both ways: the **32B** is smart enough but cold-starts in ~215–255s (only fix is a warm GPU at ~$400–800/mo, indefensible at this scale); the **7B** scale-to-zero's fine (34.5s cold) but is too weak on coordination judgment (~40/55 eval vs Mistral's 57/63). The structural squeeze — fully-open OLMo + scale-to-zero economics + good-enough quality — admits any two, not all three, and pilot scale is what makes it bite. So the running default is a tier-3 open-weight model (Mistral). Full evidence: `docs/runpod-olmo-benchmark.md`. **Re-evaluate OLMo if** a neutral provider lists an instruct variant, or the project grows enough to amortize a warm 32B.
...
- `STOP {purpose}` — mute by purpose: `STOP gleaning` (food-access) or `STOP farm help`. (Replaced the old per-activity-slug muting in the activity-model redesign.)
...
- **LLM:** abstracted via internal `LLMClient` (OpenAI-format wire protocol, opt-in Anthropic cache hints). The model is an implementation detail configured in one place (`app/config.py`), not a product feature — see the Project Constitution section for the provider-preference order. Anthropic Sonnet 4.6 remains a config-switchable fallback. The unified-agent refactor consolidated the v1 trio of LLM calls into a single `model_tier="strong"` call per inbound; `model_tier="fast"` is preserved for cheap classifier/background work.
  ...
  - OLMo (the constitution's ethical benchmark) is **not currently reachable via any hosted provider** (verified 2026-05-31). DeepInfra silently serves Gemma for an OLMo id; OpenRouter lists `allenai/olmo-3.1-32b-instruct` but with **zero live endpoints** — a real call returns HTTP 404 "No endpoints found". The only OLMo with a live OpenRouter provider is `allenai/olmo-3-32b-think`, a Think variant we avoid for concise SMS/JSON work. The `olmo-openrouter` branch stays wired (it points at the instruct slug and will 404 until a provider lists one again) but must NOT be the pilot default. The only way to actually run OLMo today is self-hosting: `LLM_PROVIDER=olmo` (`LLM_BASE_URL` defaults to `http://localhost:8000/v1` for a local vLLM/SGLang endpoint, uses HF repo ids).
