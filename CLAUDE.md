...
- **Built, unmerged**: F-043 (interactive island map — filters, drawn Vashon/Maury coastline, linked
  stand list). Availability columns now have their first reader; `open_days` is 0% populated
  island-wide, so `Open now` is season + time-of-day only. Owed: a real browser.
