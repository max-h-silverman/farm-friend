...
- **LLM provider is config-switchable.** The unified agent uses one `chat_json(model_tier="strong", …)` call per inbound and per review tick. The default is OLMo 3.1 32B Instruct through an OpenAI-compatible endpoint (`LLM_PROVIDER=olmo`, `LLM_MODEL=allenai/Olmo-3.1-32B-Instruct`); the lightweight tier defaults to `allenai/Olmo-3-7B-Instruct` for future classifier/background work. Anthropic Sonnet 4.6 is preserved as a fallback (`LLM_PROVIDER=anthropic`). Changing the default requires re-running `python -m tests.evals.runner --live` against the candidate and requiring pass-rate parity. The runner picks the provider from `LLM_PROVIDER` automatically.
