...
- [ ] **Live eval the OLMo default before pilot traffic.** Code defaults now point at `LLM_PROVIDER=olmo`, `LLM_MODEL=allenai/Olmo-3.1-32B-Instruct`, and `LLM_CLASSIFIER_MODEL=allenai/Olmo-3-7B-Instruct` through an OpenAI-compatible endpoint. Run `python -m tests.evals.runner --live` from `functions/` against the actual vLLM/SGLang/hosted endpoint and require pass-rate parity before relying on it for real SMS coordination.
...
The prior Llama/DeepInfra adapter work is still useful context for provider swaps. It cleared 53/54 deterministically (1 flake same shape as Sonnet 4.6's), but the current default has moved to OLMo and needs its own live eval before pilot traffic.
