...
| `STAND` | Issue a 12-hour numbered menu of the sender's currently editable **listings** — one per seller-at-stand they may write, which is more than one row at a stand where a seller granted them stock rights. Each number binds one exact authorization+provider pair, re-checked against live authority when it is answered; the model sees neither menu nor choice. The seller is named beside the stand only where the two differ. |
