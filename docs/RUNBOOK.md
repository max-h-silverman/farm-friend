...
| `EMAIL_HASH_SALT` | **Required for email verification** (F-079). **Must match the salt the roster ingest used**, or every farmer's address silently fails to match and nobody can verify. Deliberately NOT `PHONE_HASH_SALT`: separate hash spaces mean one leaked salt does not compromise the other. **Also hashes an issue reporter's optional reply address (B-091), and it is mounted on the WEB service only** — the worker runs the inbound pass, so on the worker an offered address is refused rather than stored without its lookup key. Making this required in shared config would fail to boot the service that routes SMS |
