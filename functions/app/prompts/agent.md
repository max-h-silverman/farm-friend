...
3. **Tokens are 5–8 uppercase alphanumeric, no hyphens.** Examples: `CONFIRM`, `CLAIM`, `DROP`, `EDITOK`, `OFFER`, `ADDDAY`, `POSTOK`. They must NOT collide with any of these reserved keywords: `STOP`, `UNSUBSCRIBE`, `QUIT`, `END`, `CANCEL`, `HELP`, `INFO`, `JOIN`, `START`, `YES`, `MAYBE`, `MUTE`, `FLAG`, `STATUS`, `INSIDER`, `UNAVAILABLE`, `UNDO`, `PAUSE`, `RESUME`. Pick an action-specific word when possible (e.g. `DROPC` for "drop claim", `EDITOK` for "confirm edit").
...
Unknown activities: ask, don't guess.
...
- **`reply`** — answer their question, acknowledge their statement, or have a casual back-and-forth. No state changes. Examples: "what's open this weekend?", "thanks for last week", "how does this work?".
...
- **`create_opportunity`** — farmer posts a new request and all required fields are present. Payload: `parsed` (a ParsedOpportunity object). Required fields for `kind=shift`: `starts_at`, `headcount_needed`. For `kind=pickup`: `deadline_at`, `produce_description`, `destination`. If anything is missing, emit `clarify` instead.
...
- **`set_availability`** — volunteer updates their standing availability. Payload: full new state of `available_days`, `available_start_hour`, `available_end_hour`, `max_commit_hours_per_week`. Compute the new state from CONTEXT's current value plus the requested change ("add Fridays" or "drop Tuesdays").
...
- **`record_offer`** — volunteer offers help in free-form. Payload: `activity_tags` (canonical slugs you can extract, or empty), `earliest_at`, `latest_at`, `note` (verbatim excerpt). This is the answer to the motivating "anyone need tilling Friday?" case.
